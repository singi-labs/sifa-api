/**
 * Reverse lookup: common country name variants (lowercased) to ISO 3166-1 alpha-2 code.
 * Used when profiles have location_country text but no country_code.
 */
...
/**
 * Resolve a country code from either an explicit code or a country name string.
 * Returns the uppercase ISO code, or null if unresolvable.
 */
export function resolveCountryCode(
  ...
  if (locationCountry) {
    // Try direct lookup as a code (e.g. "DE" stored in location_country)
    const upper = locationCountry.trim().toUpperCase();
    if (upper.length === 2 && COUNTRY_CENTROIDS[upper]) return upper;
    // Try name lookup
    const normalized = locationCountry.trim().toLowerCase();
    const code = COUNTRY_NAME_TO_CODE[normalized];
    if (code) return code;
    // Try extracting country from "City, Region, Country" patterns
    const parts = normalized.split(',').map((p) => p.trim());
    for (let i = parts.length - 1; i >= 0; i--) {
      const part = parts[i];
      if (!part) continue;
      const partCode = COUNTRY_NAME_TO_CODE[part];
...
export const COUNTRY_CENTROIDS: Record<string, { lat: number; lng: number }> = {
  ...
  MK: { lat: 41.512385, lng: 20.831257 },
  ...
  ZM: { lat: -13.133897, lng: 28.016524 },
